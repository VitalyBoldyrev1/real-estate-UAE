...
def categorize_transactions(df: pd.DataFrame, column_name: str) -> pd.DataFrame:
    category_map = {
        "Sell": "Standard Sale",
    ...
    df = df.copy()
    df[column_name + "_grouped"] = (
        df[column_name].map(category_map).fillna("Other_Transaction")
    )
    return df
