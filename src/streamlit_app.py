...
def load_model_and_artifacts():
    ...
    known_projects.update(
        load_known_values_from_file(PROJECT_NAME_PATH, known_projects)
    )
    known_master_projects.update(
        load_known_values_from_file(MASTER_PROJECT_PATH, known_master_projects)
    )
...
if model is None:
    ...
    with col1:
        ...
        trans_group_en_input = st.selectbox(
            ...
            index=TRANS_GROUP_EN_OPTIONS.index(UNKNOWN_VALUE_PLACEHOLDER)
            if UNKNOWN_VALUE_PLACEHOLDER in TRANS_GROUP_EN_OPTIONS
            else 0,
        )
        procedure_name_en_grouped_input = st.selectbox(
            "Procedure Type (Grouped):",
            options=PROCEDURE_NAME_EN_GROUPED_OPTIONS,
            index=PROCEDURE_NAME_EN_GROUPED_OPTIONS.index(UNKNOWN_VALUE_PLACEHOLDER)
            if UNKNOWN_VALUE_PLACEHOLDER in PROCEDURE_NAME_EN_GROUPED_OPTIONS
            else 0,
        )
        reg_type_en_input = st.selectbox(
            "Registration Type:",
            options=REG_TYPE_EN_OPTIONS,
            index=REG_TYPE_EN_OPTIONS.index(UNKNOWN_VALUE_PLACEHOLDER)
            if UNKNOWN_VALUE_PLACEHOLDER in REG_TYPE_EN_OPTIONS
            else 0,
        )
    ...
    with col2:
        ...
        district_input = st.selectbox(
            ...
            index=DISTRICT_OPTIONS.index(
                ...
                if "Unknown_District" not in DISTRICT_OPTIONS
                else "Unknown_District"
            )
            if (
                UNKNOWN_VALUE_PLACEHOLDER
                if "Unknown_District" not in DISTRICT_OPTIONS
        ...
        with st.expander("Help: Select Project Name (optional)"):
            selected_project_from_list = st.selectbox(
    ...
    if st.button("Predict Price", type="primary", use_container_width=True):
        ...
        try:
            ...
            st.metric(
                label="Predicted Price per sq.m. (AED)",
