...
PROCEDURE_NAME_EN_GROUPED_OPTIONS = sorted(
    ['Standard Sale', 'Mortgage', 'Grant', 'Development', 'Lease Agreement', 'Portfolio', 'Other_Transaction'])
DISTRICT_OPTIONS = sorted(['Dubai Marina & JBR', 'Al Barsha & Al Quoz', 'Eastern Dubai',
       'Meydan & Nad Al Shiba', 'Palm Jumeirah',
...
def load_model_and_artifacts():
    ...
    known_projects.update(load_known_values_from_file(KNOWN_PROJECT_NAMES_PATH, known_projects))
    known_master_projects.update(load_known_values_from_file(KNOWN_MASTER_PROJECT_NAMES_PATH, known_master_projects))
...
def create_input_dataframe(trans_group, date_val, reg_type, project_name,
                           master_project, area, proc_name_grouped, district_val,
                           known_projects_set, known_master_projects_set, unknown_placeholder):
    ...
    if not final_project_name:
        ...
        st.info(
            f"Name of '{final_project_name}' not found. '{unknown_placeholder}' will be used instead.")
        final_project_name = unknown_placeholder
    ...
    if not final_master_project_name:
        ...
        st.info(
            f"Name of '{final_master_project_name}' not found. '{unknown_placeholder}' will be used instead.")
        final_master_project_name = unknown_placeholder
    ...
    feature_order = ['trans_group_en', 'date', 'reg_type_en', 'project_name_en',
                     'master_project_en', 'procedure_area', 'procedure_name_en_grouped',
                     'district']
...
if model is None:
    st.error("The model failed to load. Please check the model path and restart the app.")
else:
    col1, col2 = st.columns(2)

    with col1:
        st.subheader("Transaction Parameters")
        trans_group_en_input = st.selectbox("Transaction Type:", options=TRANS_GROUP_EN_OPTIONS,
                                            index=TRANS_GROUP_EN_OPTIONS.index(
                                                UNKNOWN_VALUE_PLACEHOLDER) if UNKNOWN_VALUE_PLACEHOLDER in TRANS_GROUP_EN_OPTIONS else 0)
        procedure_name_en_grouped_input = st.selectbox("Procedure Type (Grouped):",
                                                       options=PROCEDURE_NAME_EN_GROUPED_OPTIONS,
                                                       index=PROCEDURE_NAME_EN_GROUPED_OPTIONS.index(
                                                           UNKNOWN_VALUE_PLACEHOLDER) if UNKNOWN_VALUE_PLACEHOLDER in PROCEDURE_NAME_EN_GROUPED_OPTIONS else 0)
        reg_type_en_input = st.selectbox("Registration Type:", options=REG_TYPE_EN_OPTIONS,
                                         index=REG_TYPE_EN_OPTIONS.index(
                                             UNKNOWN_VALUE_PLACEHOLDER) if UNKNOWN_VALUE_PLACEHOLDER in REG_TYPE_EN_OPTIONS else 0)

        st.subheader("Property Parameters")
        procedure_area_input = st.number_input("Property Area (sq.m.):", min_value=10.0, max_value=10000.0,
    ...
    with col2:
        ...
        project_name_input_str = st.text_input("Project Name:",
        ...
        with st.expander("Help: Select Project Name (optional)"):
            selected_project_from_list = st.selectbox("Search or choose a project:",
                                                      options=[""] + sorted(list(KNOWN_PROJECT_NAMES)))
            if selected_project_from_list and not project_name_input_str:
                st.caption(
                    f"Selected project: {selected_project_from_list}. You can copy it to the field above or leave it blank to use '{UNKNOWN_VALUE_PLACEHOLDER}'.")

        master_project_name_input_str = st.text_input("Developer Name:",
        ...
        with st.expander("Help: Select Developer Name (optional)"):
            selected_master_project_from_list = st.selectbox("Search or choose a developer project:",
                                                             options=[""] + sorted(list(KNOWN_MASTER_PROJECT_NAMES)))
            if selected_master_project_from_list and not master_project_name_input_str:
                st.caption(
                    f"Selected developer project: {selected_master_project_from_list}. You can copy it to the field above or leave it blank to use '{UNKNOWN_VALUE_PLACEHOLDER}'.")
    ...
    if st.button("Predict Price", type="primary", use_container_width=True):
        ...
        try:
            ...
            st.metric(label="Predicted Price per sq.m. (AED)", value=f"{predicted_price_per_sqm:,.2f}")
            st.metric(label="Estimated Total Property Price (AED)", value=f"{total_price:,.0f}")
